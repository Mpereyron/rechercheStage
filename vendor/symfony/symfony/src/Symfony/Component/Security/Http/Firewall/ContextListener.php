    ...
    public function __construct(TokenStorageInterface $tokenStorage, array $userProviders, $contextKey, LoggerInterface $logger = null, EventDispatcherInterface $dispatcher = null, AuthenticationTrustResolverInterface $trustResolver = null)
    {
        ...
        foreach ($userProviders as $userProvider) {
            if (!$userProvider instanceof UserProviderInterface) {
                throw new \InvalidArgumentException(sprintf('Model provider "%s" must implement "Symfony\Component\Security\Core\Model\UserProviderInterface".', get_class($userProvider)));
            }
        }
    ...
    protected function refreshUser(TokenInterface $token)
    {
        ...
        foreach ($this->userProviders as $provider) {
            try {
                ...
                if (null !== $this->logger) {
                    $this->logger->debug('Model was reloaded from a user provider.', array('username' => $refreshedUser->getUsername(), 'provider' => get_class($provider)));
                }
